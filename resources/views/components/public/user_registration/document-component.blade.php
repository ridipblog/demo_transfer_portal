        ...
        <div class="flex-col items-center gap-8 {{ $key == 5 ? (isset($viewData['save_data']->additional_info->pending_govt_dues) ? ($viewData['save_data']->additional_info->pending_govt_dues == 'no' ? '' : 'hidden') : 'hidden') : '' }} "
            ...
            <p class="text-center text-xs mt-4 font-medium">
                @php
                    ...
                    $doc_extension=$doc_value ? strtolower(pathinfo($doc_value->documet_location, PATHINFO_EXTENSION)) : null;
                @endphp
            ...
            <label for="{{ $value }}"
                ...
                <div
                    ...
                    <a class="set-document inline-block absolute top-1.5 left-1.5 text-blue-600 text-[.6rem] rounded-md block px-2 py-1 pdf-icon {{$doc_extension ?($doc_extension =="pdf" ?:'hidden'): 'hidden'}}"
                        target="_blank" href="{{ isset($doc_value->documet_location) ? Storage::url($doc_value->documet_location ?? null) :'' }}">View File</a>
                    <button type="button"
                        class="absolute top-1.5 right-1.5 text-red-600 text-[.6rem] rounded-md block px-2 py-1 up_doc_rmv_btn">Remove</button>
                    <img class="h-36 max-w-12/12 set-image set-document {{$doc_extension ?($doc_extension !="pdf" ?:'hidden'): 'hidden'}}" src="{{ isset($doc_value->documet_location) ? Storage::url($doc_value->documet_location ?? null) :'' }}"
                        alt="">
                    <i class="bi bi-filetype-pdf text-5xl pdf-icon {{$doc_extension ?($doc_extension =="pdf" ?:'hidden'): 'hidden'}}"></i>
                </div>
